...
class TwitchWebhook extends EventEmitter {
	...
	constructor(options = {}) {
        ...
        if(!options.authorizationKey){
            throw new errors.FatalError("Twitch Authorization key is not provided")
        }
		...
		if (Object.keys(options.https).length) {
			this._server = https.createServer(options.https, this._requestListener.bind(this));
		} else {
		...
		if (options.listen.autoStart !== false) {
			this.listen();
		}
	}
	...
	listen(...args) {
		...
		return new Promise(resolve => {
			if (!args.length) {
				this._server.listen(this._options.listen.port, this._options.listen.host, () => resolve());
			} else {
	...
	_request(mode, topic, options) {
		...
		requestOptions.headers = {
            ...
            Authorization: `Bearer ${this._options.authorizationKey}`
		};
		requestOptions.qs = {
		...
		if (this._options.secret) {
			const secret = crypto.createHmac("sha256", this._options.secret).update(topic).digest("hex");
	...
	_processUpdates(request, response) {
		...
		let signature;
		if (this._options.secret) {
			signature = request.headers["x-hub-signature"] && request.headers["x-hub-signature"].split("=")[1];
		...
		request.on("end", () => {
			...
			if (this._options.secret) {
				let storedSign = crypto.createHmac("sha256", this._subscriptions[endpoint].secret).update(body).digest("hex");
