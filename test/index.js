...
describe("TwitchWebhook", () => {
	...
	before(() => {
		return helpers.startMockedServer(apiPort).then(() => {
    ...
    it("should throw FatalError if the Twitch Authorization key is not provided", done => {
		try {
			new TwitchWebhook({
                // eslint-disable-line no-new
                client_id: clientId,
			...
			assert(err instanceof errors.FatalError);
			assert(err.message === "Twitch Authorization key is not provided");
			done();
		}
	});
	...
	it('should set "host" and "post" if one of them is undefined', done => {
		...
		tempWebhook.on("listening", () => {
			helpers
				.hasStartedListening(`http://0.0.0.0:8443/`)
				.then(() => done())
				.catch(done)
		...
		describe("POST method", () => {
			...
			describe("secret support", () => {
				...
				it("returns 200 response code if everything is ok", () => {
					...
					const signature = crypto.createHmac("sha256", storedSign).update(body).digest("hex");
